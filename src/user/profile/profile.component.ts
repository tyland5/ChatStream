...
export class Profile implements OnInit, OnDestroy{
  ...
  constructor(private profileService: ProfileService, private sanitizer: DomSanitizer, private registerService: CreateAccountService, private personalUserInfoService: PersonalUserInfoService){}

  ngOnInit(): void {
    this.personalUserInfoSubscription = this.personalUserInfoService.userInfo.subscribe(info=> this.userInfo = info)
  }
  ...
  saveProfileChanges(){
    ...
    this.registerService.checkUsernameAvailable(this.userInfo.username).subscribe(isAvailable=>{
      ...
      if(this.newPfp !== ''){
        const newPfpName: string = this.userInfo.id + "pfp_" + Date.now().toString()
